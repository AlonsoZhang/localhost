...
function maintop($title,$showtop = true) {
    ...
    echo "<!DOCTYPE html>\n<meta name='robots' content='noindex,follow' />\n<head>\n<meta name='viewport' content='width=device-width, initial-scale=1'/>\n"
        ...
        .'<script type="text/javascript" src="edit_area/jquery-1.7.1.min.js"></script>'
        ."</head>\n"
        ."<body>\n"
...
function home() {
    ...
    if ($stylesheet !== "." && $stylesheet !== ".." ) {
        ...
        $folder = uCode($folder);
        $trontd = "<tr width=100% onclick='st=document.getElementById(\"$stylesheet\").checked;if(st==true){document.getElementById(\"$stylesheet\").checked=false;this.style.backgroundColor=\"\";}else{document.getElementById(\"$stylesheet\").checked=true;this.style.backgroundColor=\"#696969\";}'><td><svg width='21' height='21'>";
...
function edit($fename) {
    ...
    if (file_exists($file)) {
        ...
        echo "<form action='{$meurl}?op=save' method='post' ><div class='title'>编辑文件 {$folder}{$fename}\n"
            ...
            .'</select></span></div>'
            ."<textarea rows='24' name='ncontent' id='ncontent' style=’bgcolor:#FFFF00‘>"
            .htmlspecialchars($contents)
            ."</textarea>"
            ."<br>\n"
            ."<input type='hidden' name='folder' value='{$folder}'>\n"
            ."<input type='hidden' name='fename' value='{$fename}'>\n"
            ."<input type='submit' name='Dosave' value='保存' class='right button mob'>\n"
            ."</form>\n"
            .'<script language="javascript" type="text/javascript">
              editAreaLoader.init({
              id : "ncontent"        // textarea id
              ,syntax: "css"         // syntax to be uses for highgliting
              ,start_highlight: true    // to display with highlight mode on start-up
              ,toolbar: "save,fullscreen,word_wrap,search, go_to_line, |, undo, redo, |, select_font, |, change_smooth_selection,syntax_selection, highlight, reset_highlight, |, help"
              ,load_callback: "my_load"
              ,save_callback: "my_save"
              });
              function my_save(id, content)
              {
                Url="'.$_SERVER['PHP_SELF'].'?op=save";
           $.post(Url,{
           encode:"'.$encode.'",
           ncontent:content,
           folder:"'.$folder.'",
